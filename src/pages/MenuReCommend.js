...
function MenuReCommend() {
  ...
  const selectCategory = async (category, index) => {
    ...
    try {
      ...
      if (snapshot.exists()) {
        ...
        setSelectedCategoryIndex(index); // 선택한 1차 카테고리의 인덱스 저장
  ...
  const selectSubCategory = useCallback((index) => {
    ...
    if (!selectedCategory || !subcategories[index] || !filteredMenus.length) {
      ...
      setFinalList(temp);
      setSelectedSubCategoryIndex(index); // 선택한 1차 카테고리의 인덱스 저장
  ...
  useEffect(() => {
    ...
    }, [filteredMenus, selectSubCategory, selectedSubCategory, setSelectedSubCategoryIndex]);
    ...
    <>
      ...
      <div className="lists-container">
        <ul className='first_category'>
          ...
          <li key={index} className={selectedSubCategoryIndex === index ? 'selected' : ''} onClick={() => selectSubCategory(index)}>
            {subcategory}</li> ))}
          </ul>
          {finalMenu && (
              <Modal className= 'foodModal' isOpen={isModalOpen} onRequestClose={closeModal}>
                <div>
                <img className='reject' src="https://i.ibb.co/YZbWQM5/reject.png" alt="reject" onClick={closeModal}/>
                ...
                {finalMenu && (<p className="menu-description">
                    ...
                    {!votedMenus[finalMenu.name] && (
                      <img className="vote" src="https://i.ibb.co/4VXmN4x/like-1.png" onClick={() => upvote()} />
                    )}
